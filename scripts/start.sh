...
if [ -f "./next-service-dist/server.js" ]; then
    ...
    if [ "$DATABASE_URL" = "$DEFAULT_PACKAGED_DATABASE_URL" ]; then
        if [ ! -f "$DEFAULT_PACKAGED_DB_PATH" ]; then
            echo "Error: Database file not found at $DEFAULT_PACKAGED_DB_PATH"
            exit 1
        fi
...
if [ -f "../Caddyfile" ] || [ -f "./Caddyfile" ]; then
    echo "Starting Caddy..."
    CADDYFILE="./Caddyfile"
    [ ! -f "$CADDYFILE" ] && CADDYFILE="../Caddyfile"
    ...
    echo ""
    echo "All services started! (no Caddy)"
    echo ""
    echo "Press Ctrl+C to stop all services"
    echo ""
